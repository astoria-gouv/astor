...
async fn test_currency_issuance() {
    ...
    let tx_id = system.issue_currency("root", &recipient_account, 1000, &signature).unwrap();
    
    // Verify balance
    let balance = system.account_manager.get_balance(&recipient_account).unwrap();
    ...
    assert!(matches!(transaction.transaction_type, astor_currency::transactions::TransactionType::Issuance { .. }));
}
...
async fn test_account_transfer() {
    ...
    let from_account = system.account_manager.create_account(Some(from_keypair.public_key()));
    ...
    system.issue_currency("root", &from_account, 1000, &admin_signature).unwrap();
    
    // Transfer between accounts
    let transfer_signature = from_keypair.sign(format!("transfer_from_{}", from_account).as_bytes());
    let tx_id = system.transfer(&from_account, &to_account, 300, &transfer_signature).unwrap();
    
    // Verify balances
    assert_eq!(system.account_manager.get_balance(&from_account).unwrap(), 700);
    assert_eq!(system.account_manager.get_balance(&to_account).unwrap(), 300);
    
    // Verify transaction
    let transaction = system.transaction_manager.get_transaction(&tx_id).unwrap();
    assert!(matches!(transaction.transaction_type, astor_currency::transactions::TransactionType::Transfer { .. }));
}
...
async fn test_ledger_integrity() {
    ...
    let signature = root_keypair.sign(b"issue_currency");
    system.issue_currency("root", &account1, 1000, &signature).unwrap();
    system.issue_currency("root", &account2, 500, &signature).unwrap();
...
async fn test_insufficient_funds() {
    ...
    assert!(matches!(result.unwrap_err(), astor_currency::AstorError::InsufficientFunds));
}
