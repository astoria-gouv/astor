...
impl MetricsCollector {
    pub async fn new(config: &MetricsConfig) -> Result<Self, AstorError> {
        ...
        let http_requests_total =
            register_int_counter!("astor_http_requests_total", "Total number of HTTP requests")
                .map_err(|e| {
                    AstorError::MonitoringError(format!("Failed to register metric: {}", e))
                })?;
        ...
        let active_accounts =
            register_int_gauge!("astor_active_accounts", "Number of active accounts").map_err(
                |e| AstorError::MonitoringError(format!("Failed to register metric: {}", e)),
            )?;
        ...
        let memory_usage = register_gauge!("astor_memory_usage_bytes", "Memory usage in bytes")
            .map_err(|e| {
                AstorError::MonitoringError(format!("Failed to register metric: {}", e))
            })?;

        let cpu_usage = register_gauge!("astor_cpu_usage_percent", "CPU usage percentage")
            .map_err(|e| {
                AstorError::MonitoringError(format!("Failed to register metric: {}", e))
            })?;
    ...
    pub async fn record_business_metric(&self, metric: BusinessMetric) {
        match metric {
            BusinessMetric::TransactionCreated {
                amount,
                transaction_type,
            } => {
                self.transactions_total.inc();
                tracing::info!(
                    "Transaction created: {} ASTOR ({})",
                    amount,
                    transaction_type
                );
            }
            BusinessMetric::TransactionCompleted {
                amount,
                duration_ms,
            } => {
                ...
                tracing::info!(
                    "Transaction completed: {} ASTOR in {}ms",
                    amount,
                    duration_ms
                );
            }
            ...
            BusinessMetric::SecurityViolation {
                violation_type,
                severity,
            } => {
